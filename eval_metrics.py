...
def load_data(json_path):
    ...
    preds = [x["synthesized_report"].strip() for x in data]
    return refs, preds
...
def compute_rouge(refs, preds):
    ...
    return scores  # returns dict with rouge1, rouge2, rougeL, etc.
...
if __name__ == "__main__":
    # Path to your sample output JSON
    path = "/mnt/storage/home/zy0058@students.ad.unt.edu/rrg/outputs/synthesis/synthesized_reports.json"

    refs, preds = load_data(path)

    print("🔹 BLEU Score:")
    print(f"  BLEU: {compute_bleu(refs, preds):.4f}")
